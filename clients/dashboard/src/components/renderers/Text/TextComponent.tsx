...
/**
 * TextComponent - Demonstrates DataRegistry integration
 *
...
export const TextComponent: React.FC<ItemRendererProps> = ({
  ...
  // State to track the current value from DataRegistry
  ...
  useEffect(() => {
    ...
    if (!dataName) {
      console.warn('[TextComponent] No content or data name provided');
      return;
    }
    ...
    const handleDataChange = (newValue: any, oldValue: any) => {
      console.log(`[TextComponent] Data "${dataName}" changed:`, { oldValue, newValue });

      // Update component display with new value
    ...
    return () => {
      ...
      console.log(`[TextComponent] Unsubscribed from "${dataName}"`);
    };

    // Re-run effect if item.content changes (data name changes)
  }, [item.content]);
