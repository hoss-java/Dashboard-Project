...
//    });
  ...
  describe('DELETE Requests', () => {
    ...
    test('should make DELETE request', async () => {
      ...
      expect(global.fetch).toHaveBeenCalledWith(
        expect.stringContaining('/rooms/123'), // or just check the full URL
  ...
  describe('Proxy URL Handling', () => {
    ...
    test('should properly encode endpoint in proxy URL', async () => {
      ...
      // or match the full expected URL format:
      expect(calledUrl).toBe('http://localhost:3280/templateApp/api/auth/me');
    });
    ...
    test('should encode complex endpoints with parameters', async () => {
      ...
      const calledUrl = (global.fetch as jest.Mock).mock.calls[0][0];
      expect(calledUrl).toContain('/rooms/123/messages');
