...
class ApiClient {
  ...
  constructor(config: ApiClientConfig = {}) {
    ...
    this.phpProxyURL = config.phpProxyURL || API_CONFIG.phpProxyURL;
    this.usePhpProxy = config.usePhpProxy ?? API_CONFIG.usePhpProxy;
  ...
  async request(
    ...
    if (this.usePhpProxy) {
      // PHP proxy mode: endpoint as parameter
      requestUrl = `${this.phpProxyURL}?endpoint=${encodeURIComponent(endpoint)}`;
    } else {
      // Lighttpd proxy mode: endpoint as path
      requestUrl = `${this.proxyURL}${endpoint}`;
    }
